const API_BASE = 'https://sky-foundation-certifyme-assignment-2.onrender.com';
...
function handleLogout() {
    fetch(`${API_BASE}/logout`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } })
        .then(() => {
        ...
        if (response.ok) {
            ...
            if (grid) {
                ...
                if (data.data.length === 0) {
                    grid.innerHTML = '<p class="empty-state">No opportunities created yet.</p>';
                } else {
                    data.data.forEach(op => renderOpportunityCard(op));
                }
            }
        }
    } catch (err) {
        showToast('Error fetching opportunities');
    }
}
...
function renderOpportunityCard(op) {
    ...
    const applicantsCount = op.max_applicants ? `${op.max_applicants} applicants max` : 'No limit';
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer" style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="display:flex; gap: 8px;">
                <button class="view-course-btn view-btn" style="width: auto; padding: 6px 12px; font-size: 12px;">View</button>
                <button class="view-course-btn edit-btn" style="width: auto; padding: 6px 12px; font-size: 12px; background: #2b5c8f;">Edit</button>
                <button class="view-course-btn delete-btn" style="width: auto; padding: 6px 12px; font-size: 12px; background: #a33232;">Delete</button>
            </div>
        </div>
    `;
    ...
    card.querySelector('.delete-btn').addEventListener('click', async () => {
        if (confirm('Are you sure you want to delete this opportunity?')) {
            try {
                const response = await fetch(`${API_BASE}/api/opportunities/${op.id}`, {
                    method: 'DELETE',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' }
                });
...
document.getElementById('opportunityForm').addEventListener('submit', async function (e) {
    ...
    // collect values
    const title = document.getElementById('oppName').value.trim();
    ...
    let max_applicants = document.getElementById('oppMaxApplicants').value.trim();
    max_applicants = max_applicants ? parseInt(max_applicants, 10) : null;

    // basic validation
    if (!title || !duration || !start_date || !description || !skillsRaw || !category || !future_opportunities) {
        showToast('Please fill all required fields');
        return;
    }

    const payload = {
        title, duration, start_date, description, skills: skillsRaw, category, future_opportunities, max_applicants
    };
    ...
    try {
        ...
        if (editingOpportunityId) {
            url = `${API_BASE}/api/opportunities/${editingOpportunityId}/edit`;
            method = 'PUT';
        }
        ...
        if (response.ok) {
            ...
            fetchOpportunities(); // Refresh grid
        } else {
...
document.getElementById('loginForm').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            setTimeout(() => showDashboard(email, data.admin.full_name), 1200);
            this.reset();
        }
    } catch (error) {
